...
const VISUAL_RULE =
  ...
  '{"kind":"structure","notation":"smiles"|"reaction-smiles","value":"CC(=O)Oc1ccccc1C(=O)O","highlight":[0,1],"carbons":"skeletal"|"all","conditions":"…"}: a molecule or a reaction, given ONLY as canonical notation; never coordinates, never a drawing; an open attachment point is "*", so a generic alcohol is "*O", a carboxylic acid "*C(=O)O" and an amide "*C(=O)N": "R" is not a SMILES atom and a value containing one draws nothing at all. For a compound that HAS A NAME, give {"kind":"structure","compound":"aspirin"} with no value: the structure is looked up from a chemical database and drawn from what it returns, which is more trustworthy than notation written from memory. Use "value" only for a generic group or a fragment no database holds. "highlight" names the parts a step is ABOUT, counting heavy atoms from zero in the value: a bare number is an atom and a PAIR of numbers is the bond between them, so "highlight":[0,[1,2]] marks atom 0 and the 1-2 bond. Put several species in one value separated by "." so they share one index space ("[OH-].CBr" makes O atom 0, C atom 1, Br atom 2); a bond highlight works only on "smiles", never on "reaction-smiles". NEVER DRAW ELECTRON MOVEMENT: there are no curly arrows, no fishhook arrows and no lone-pair dots, and a request for one draws nothing at all. For a mechanism, highlight the atom being attacked and the bond that breaks, and SAY IN ORDINARY SENTENCES what the electrons do, one step at a time ("the oxygen lone pair attacks the carbonyl carbon; the C-Br bond breaks and its electrons move onto bromine"); ' +
  '{"kind":"mechanism","steps":[{"value":"NCCOc1ccc(cn1)[N+](=O)[O-]","label":"NaH"},{"value":"[NH-]CCOc1ccc(cn1)[N+](=O)[O-]","highlight":[0,[3,4]]},{"value":"OCCNc1ccc(cn1)[N+](=O)[O-]"}]}: a whole reaction as ONE connected scheme, 2 to 6 frames flowing across the page with a reaction arrow between each pair, the way a textbook prints a mechanism. Each step is a smiles value with its own "highlight" in its own index space, and "label" is what is written on the arrow LEAVING that step (a reagent, a condition). Reach for this rather than several separate structure visuals whenever the steps belong to one reaction: separate cards read as separate molecules, a scheme reads as a reaction going somewhere. Then explain the steps in your own sentences underneath, because the picture shows WHAT the species are and only your words can say WHY; ' +
