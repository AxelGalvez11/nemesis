...
test("🔴 both source surfaces draw the mark, or one of them is still a wall of identical rows", () => {
  ...
  const preview = readFileSync(new URL("../../components/workspace/learn/source-preview.tsx", import.meta.url), "utf8");
  assert.match(controls, /fileMark\(source\.title, source\.kind\)/, "the sources shelf is back to one glyph for everything");
  assert.match(preview, /fileMark\(source\.title, source\.kind\)/, "the reading pane's tabs are back to one glyph for everything");
  assert.ok(!/name="file"\s+size="14px"/.test(preview), "the tab strip still hard-codes the generic page glyph");
});
