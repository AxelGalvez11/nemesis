// Turning the model's live reasoning into ONE readable line. WEB port of the
// phone's apps/mobile/src/lib/reasoning-preview.ts, kept in parity by hand so
// both surfaces read the same working-out the same way.
//
...
// arrives on the wire, and until now web threw it away and showed a static
// "Thinking" shimmer instead. This file picks the fragment of that stream worth
// showing at any given moment.
//
 ...
 *
 * Hand-rolled rather than a split on a lookbehind pattern: this stays byte-for-
 * byte identical to the phone's version, whose JS engine (Hermes) has no
 * lookbehind support — keeping the two in exact parity is worth more than a
 * marginally shorter regex that could drift.
 *
