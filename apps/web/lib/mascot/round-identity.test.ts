...
test("the signature jump has anticipation, flight, impact, rebound and settle", () => {
  ...
  assert.ok(anticipation.body.stretch > 1, "anticipation should widen");
  assert.ok(anticipation.body.squash < 1, "anticipation should compress vertically");

  assert.ok(apex.lift < -6, "apex should be visibly above rest");
  assert.ok(Math.abs(apex.body.stretch - 1) < 0.08, "apex should return near round");

  assert.ok(impact.body.stretch > 1.12, "landing should have a broad squish");
  assert.ok(impact.body.squash < 0.9, "landing should compress vertically");
  ...
  assert.ok(Math.abs(settled.body.stretch - 1) < 1e-12, "jump should settle to round width");
  assert.ok(Math.abs(settled.body.squash - 1) < 1e-12, "jump should settle to round height");
