...
// Which design a deck wears, remembered for the learner.
//
// 🔴 KEPT ON THE DEVICE, NOT IN THE DATABASE. A design is a preference about how a file looks
...
// changing a design writes nothing, touches no row's updated_at, and cannot reorder the
...
import { DEFAULT_DECK_DESIGN } from "./deck-designs";

// v2: the v1 ids named colourways that no longer exist (see deck-designs.ts).
const KEY = "nemesis.deck.design.v2";
/** Enough history to cover any real library; beyond it the oldest choices fall away. */
const MAX_REMEMBERED = 300;

interface Stored {
  /** The design new decks and unseen decks use — the last one the learner picked. */
  fallback?: string;
...
/** The design this deck should be built in. */
export function readDeckDesignChoice(deckKey?: string | null): string {
  const stored = read();
  const mine = deckKey ? stored.byDeck?.[deckKey] : undefined;
  return mine ?? stored.fallback ?? DEFAULT_DECK_DESIGN;
}

/** Remember a pick: for this deck, and as the default for the next one. */
export function writeDeckDesignChoice(designId: string, deckKey?: string | null): void {
  if (typeof window === "undefined") return;
  const stored = read();
  const byDeck = { ...(stored.byDeck ?? {}) };
  if (deckKey) byDeck[deckKey] = designId;
  const keys = Object.keys(byDeck);
  for (const stale of keys.slice(0, Math.max(0, keys.length - MAX_REMEMBERED))) delete byDeck[stale];
  try {
    window.localStorage.setItem(KEY, JSON.stringify({ byDeck, fallback: designId } satisfies Stored));
  } catch {
