/**
 * Written work, still working: ink on the page → real Gemini → verdict → durable evidence, with the
 * modality and the diagnosis intact.
 *
 * 🔴 A REGRESSION PROOF, NOT A REDESIGN. The writing sheet already produced a real durable row
 * before this branch existed. What changed underneath it is the composer's routing and the way a
 * screen ends, and both touch the sheet: `promptId` is gated on the intent now, and the pencil is
 * gated on the session. This exists to show that path still lands.
 *
 * 🔴 THE INK IS DRAWN AS TEXT ONTO THE REAL CANVAS ELEMENT, AND THAT IS STATED RATHER THAN GLOSSED.
 * Mouse strokes cannot produce legible working, so the harness paints the answer onto the sheet's
 * own 2D context. Everything after that is untouched: the sheet reads its own canvas, the real
 * Gemini vision call reads the image, the real judge reads the transcription, and the row is written
 * by the real evidence path. What is NOT proven here is stroke capture from a pointing device.
 *
...
async function main(): Promise<void> {
  ...
  // ── Get to a question, then reach for the page ────────────────────────────
  const asked = await waitForQuestion(page);
  await refreshExpected();
  const known = lookUp(asked);
  check("W-question", Boolean(known), known ? `“${asked.slice(0, 80)}”` : "the harness cannot identify this question");
  if (!known) { await browser.close(); process.exit(1); }

  // 🔴 THE PENCIL IS GATED ON THE SESSION, NOT ON THE QUESTION — `inSession`, which this branch now
  // derives from the composer intent. If that derivation broke, this control is simply not there.
  const pencil = page.getByRole("button", { name: "Open a page to work on" });
  check("W-pencil", (await pencil.count()) > 0, "the writing surface is reachable from the composer");
  if (!(await pencil.count())) { await browser.close(); process.exit(1); }
  await pencil.first().click();

  const sheet = page.getByRole("img", { name: "Work out your answer here" });
  const canvasEl = page.locator('canvas[aria-label="Work out your answer here"]');
  check("W-sheet", (await canvasEl.count()) > 0, "the page opened");
  void sheet;

  // 🔴 REAL STROKES FIRST, AND THEY ARE NOT DECORATION. The sheet enables `Hand this in` from
  // `hasInk`, which its own pointer handlers set — painting straight onto the 2D context left the
  // control DISABLED and the page, as far as the component was concerned, blank. So the harness
  // actually draws: pointer down, move, up, on the real element.
  const box = (await canvasEl.first().boundingBox())!;
  await page.mouse.move(box.x + 60, box.y + 200);
  await page.mouse.down();
  for (let i = 0; i < 22; i += 1) {
    await page.mouse.move(box.x + 60 + i * 9, box.y + 200 + Math.sin(i / 2) * 22);
  }
  await page.mouse.up();

  // Real working, painted onto the real sheet.
  const written = `${known.answer}`;
  await page.evaluate((text) => {
    const element = document.querySelector('canvas[aria-label="Work out your answer here"]') as HTMLCanvasElement | null;
    if (!element) throw new Error("no sheet");
    const ctx = element.getContext("2d");
    if (!ctx) throw new Error("no context");
    ctx.fillStyle = "#111111";
    ctx.font = "44px Georgia, serif";
    ctx.fillText(text, 40, 90);
    // A second line of working, so the page is not one bare token.
    ctx.font = "32px Georgia, serif";
    ctx.fillText("(from the table in the lecture)", 40, 150);
  }, written);
  note(`wrote on the page: “${written}”`);

  // 🔴 THE HAND-IN CONTROL EXISTS ONLY WHILE A QUESTION IS OPEN (`promptId`), WHICH IS THE WHOLE
  // LIFECYCLE RULE. Its absence would mean the sheet had become scratch paper mid-question.
  const handIn = page.getByRole("button", { name: /^Hand this in$/i });
  const handInCount = await handIn.count();
  if (handInCount === 0) {
    const labels = await page.evaluate(() =>
      [...document.querySelectorAll("button")].map((b) => `${(b.textContent ?? "").trim()}|${b.getAttribute("aria-label") ?? ""}`).filter(Boolean));
    note(`sheet buttons: ${JSON.stringify(labels)}`);
  }
  check("W-hand-in", handInCount > 0, `${handInCount} hand-in control(s) while a question is open`);
  if (handInCount === 0) { await browser.close(); process.exit(1); }
  await handIn.first().click();

  const rows = await waitForRow(userId, 1, 180_000);
  check("W-row", rows.length >= 1, rows.length ? `${rows.length} durable row(s)` : "NO ROW — written work never reached the judge");
  if (!rows.length) { await browser.close(); process.exit(1); }
  const w = rows[0]!;
  check("W-modality", w.response_modality === "written", `response_modality=${w.response_modality}`);
  check("W-vision-read", Boolean(w.response_text && w.response_text.length > 0), `Gemini read: ${JSON.stringify((w.response_text ?? "").slice(0, 80))}`);
  check("W-judged", w.verdict !== null, `verdict=${w.verdict} · confidence=${w.confidence} · error_type=${w.error_type}`);
  check("W-right-objective", known.identityKeys.includes(w.learning_objectives?.identity_key ?? ""), `attached to an objective the question names`);

  const evidence = await evidenceFor(userId);
  const state = projectAll(evidence);
  const mine = [...state.values()];
  check("W-model", mine.length > 0 && mine.every((one) => one.evidenceCount > 0), `${mine.map((one) => `${one.status}×${one.evidenceCount}`).join(", ")}`);
