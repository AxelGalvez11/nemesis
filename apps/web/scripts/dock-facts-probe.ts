...
async function main(): Promise<void> {
  ...
  const facts = await page.evaluate(`(() => {
    ...
    return {
      dock: r(dock),
      dockStyle: dock ? { position: getComputedStyle(dock).position, bottom: getComputedStyle(dock).bottom, left: getComputedStyle(dock).left, transform: getComputedStyle(dock).transform } : null,
      anchor: r(anchor),
      anchorExists: !!anchor,
      offsetParent: parent ? parent.tagName + "." + String(parent.className).slice(0, 40) : null,
      offsetParentRect: r(parent),
