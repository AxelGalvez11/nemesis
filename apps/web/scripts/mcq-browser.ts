/**
 * Adaptive multiple choice, proven in a real browser — reached, not forced.
 *
 * 🔴🔴 NOTHING HERE STAGES AN MCQ. `recognitionPreferred` needs TWO consecutive unresolved unaided
 * attempts on ONE objective, read out of `learner_evidence`, plus a choice set whose distractors are
 * grounded in something real — a misconception this learner actually showed, or a sibling the source
 * itself declared. So the harness does the only thing a learner can do: it answers, wrongly and
 * consistently, and waits for the runtime to decide options are worth offering. If they never
 * appear, that is the answer and this reports it rather than reaching for an override.
 *
 * 🔴 IT WAS UNREACHABLE BY CONSTRUCTION UNTIL TWO DEFECTS WERE FIXED. Typed answers were routed into
 * `begin()` and never recorded, so the streak could not accumulate; and a wrong answer landed on a
 * correction screen with no way forward, so a second attempt could not be made. Both are fixed on
 * this branch. This is the proof that the path they blocked is now open.
 *
...
import { projectAll, satisfies } from "@/lib/learn/learner-evidence";
...
/**
 * The options a recognition task renders, or the question a retrieval one does.
 *
 * 🔴 SCOPED INSIDE THE POLICY REGION, NEVER A BARE `ul > li > button`. That selector matched the
 * SIDEBAR on a previous run — "New canvas / Library / Calendar / Stats" were read as multiple-choice
 * options and the harness tapped Stats. The recognition list is the one that sits at the canvas
 * column width, immediately under the question.
 */
async function waitForOptionsOrQuestion(
  page: Page,
  timeoutMs = 180_000,
): Promise<{ kind: "options"; asked: string; options: string[] } | { kind: "question"; asked: string; options: [] }> {
  const until = Date.now() + timeoutMs;
  for (;;) {
    const options = await page.evaluate(() =>
      [...document.querySelectorAll('ul[class*="canvas-column"] li button')]
        .map((b) => (b.textContent ?? "").trim())
        .filter(Boolean),
    );
    if (options.length >= 2) {
      return { asked: (await page.locator("h2").first().innerText()).trim(), kind: "options", options };
    }
    const answering = await page.evaluate(() => {
      const box = document.querySelector("textarea");
      return Boolean(box && (box as HTMLTextAreaElement).placeholder.startsWith("Type your answer"));
    });
    if (answering) return { asked: (await page.locator("h2").first().innerText()).trim(), kind: "question", options: [] };
    if (Date.now() > until) {
      const seen = (await page.locator("body").innerText()).replace(/\n{2,}/g, " / ").slice(0, 300);
      throw new Error(`neither options nor a question after ${Math.round(timeoutMs / 1000)}s. On screen: ${seen}`);
    }
    await advance(page);
    await new Promise((r) => setTimeout(r, 1_500));
  }
...
async function tapOption(page: Page, text: string): Promise<void> {
  await page.locator('ul[class*="canvas-column"] li button', { hasText: text }).first().click();
}
...
async function main(): Promise<void> {
  ...
  // 🔴 THE ARM IS PINNED, THE QUESTION TYPE IS NOT. `?teacher=` is the product's own control for
  // choosing between two SHIPPED controllers; nothing here asks for a multiple-choice question.
  // `nemesis_policy` still has to decide, from this learner's evidence, that recognition is the
  // right next action — and refuse if `optionsFor` cannot build an honest set.
  //
  // Measured first on the default arm: fourteen consecutive unaided misses climbed the ladder to
  // `cued` and `llm_teacher` never chose `recognise`, though it can and was handed real options.
  const teacher = process.env.TEACHER ?? "nemesis_policy";
  await page.goto(`${ORIGIN}/learn?c=${canvasId}&teacher=${teacher}`, { waitUntil: "domcontentloaded" });



  const browserLog: string[] = [];

  // ── Answer wrongly, consistently, until the runtime offers options ────────
  //
  // 🔴 THE SAME WRONG BELIEF EVERY TIME, BECAUSE A MISCONCEPTION IS WHAT MAKES A DISTRACTOR REAL.
  // `choiceCandidatesFrom` builds `held_misconception` grounds out of the learner's OWN recorded
  // misconceptions; a different wrong answer each turn produces scattered ones and never a set the
  // runtime is willing to stage. This is also what a real learner with a stable wrong model does.
  ...
  for (; turns < MAX_TURNS; turns += 1) {
    const seen = await waitForOptionsOrQuestion(page);
    if (seen.kind === "options") {
      options = seen.options;
      askedAtOptions = seen.asked;
      break;
    }
    await answer(page, WRONG);
    await waitForRow(userId, turns + 1, 90_000);
    await advance(page);
  }

  check("MCQ-appeared", options.length > 0, options.length
    ? `options offered after ${turns} unaided attempt(s): ${options.length} on screen`
    : `no options after ${turns} attempts — the adaptive path did not choose recognition`);
  if (!options.length) {
    const rows = await waitForRow(userId, 1, 5_000);
    note(`rows so far: ${rows.map((r) => `${r.verdict}/${r.scaffold_rung}`).join(", ")}`);
    await browser.close();
    if (process.env.KEEP_LEARNER !== "1") await fetch(`${SB}/auth/v1/admin/users/${userId}`, { method: "DELETE", headers: svc });
    console.log(`\n${failures === 0 ? "ALL CHECKS PASSED" : `${failures} CHECK(S) FAILED`}`);
    process.exit(1);
  }

  note(`asked: “${askedAtOptions.slice(0, 90)}”`);
  for (const option of options) note(`  option: ${option}`);

  const before = await waitForRow(userId, 1, 5_000);
  await refreshExpected();
  const known = lookUp(askedAtOptions);
  check("MCQ-question-known", Boolean(known), known ? `correct answer: “${known.answer}”` : "the harness cannot identify this question");

  // ── Tap a DISTRACTOR ─────────────────────────────────────────────────────
  const distractor = options.find((one) => one.trim() !== known?.answer.trim());
  check("MCQ-has-distractor", Boolean(distractor), distractor ? `tapping “${distractor}”` : "every option is the correct answer");
  if (!distractor) { await browser.close(); process.exit(1); }
  await tapOption(page, distractor);

  const after = await waitForRow(userId, before.length + 1, 120_000);
  check("MCQ-row", after.length === before.length + 1, `${after.length - before.length} new durable row(s)`);
  const tapped = after[after.length - 1]!;
  // 🔴 THE EXACT OPTION, VERBATIM. Without this the durable record would say only "wrong", which is
  // precisely the objection multiple choice has always deserved.
  check("MCQ-exact-distractor", tapped.response_text === distractor, `response_text=${JSON.stringify((tapped.response_text ?? "").slice(0, 70))}`);
  check("MCQ-not-typed", tapped.response_modality === null, `response_modality=${tapped.response_modality} — a tap is not typed, spoken or written`);
  // 🔴 RECOGNITION IS WEAKER EVIDENCE THAN PRODUCTION, AND THE ROW HAS TO SAY SO.
  check("MCQ-rung", tapped.scaffold_rung === "recognition", `scaffold_rung=${tapped.scaffold_rung}`);
  check("MCQ-wrong", tapped.demonstration_obtained === true && tapped.verdict !== "strong" && tapped.verdict !== "understood", `verdict=${tapped.verdict}`);

  // ── Recognition must not become unjustified mastery ──────────────────────
  const evidence = await evidenceFor(userId);
  const state = projectAll(evidence);
  const forTapped = [...state.values()].find((one) => one.objectiveIdentityKey === tapped.learning_objectives?.identity_key);
  check("MCQ-model-read", Boolean(forTapped), forTapped ? `status ${forTapped.status} over ${forTapped.evidenceCount} row(s)` : "the tap never reached the learner model");
  if (forTapped) {
    check(
      "MCQ-not-mastery",
      !satisfies(forTapped, "independent"),
      `a pick does not satisfy unaided production (highest rung demonstrated: ${forTapped.demonstratedAt ?? "none"})`,
    );
  }

  // ── The policy responds to it ────────────────────────────────────────────
  await advance(page);
  const next = await waitForOptionsOrQuestion(page, 90_000).catch(() => null);
  check("MCQ-policy-responds", next !== null, next ? `next: ${next.kind} — “${next.asked.slice(0, 70)}”` : "nothing followed the tap");
  ...
  if (consoleErrors.length) note(`browser console errors: ${consoleErrors.slice(0, 4).join(" | ")}`);
  if (browserLog.length) note(browserLog.join(" | "));
