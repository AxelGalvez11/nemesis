...
function stripsSheet(a: Avatar, frames = 8, cw = 96, ch = 96): string {
  const nameW = 150, W = nameW + frames * cw + 20, H = ANIMATIONS.length * (ch + 6) + 54;
  const body = [`<text x="18" y="32" font-family="ui-sans-serif,system-ui,sans-serif" font-size="20" font-weight="600" fill="#111">${esc(a.name)} — ${ANIMATIONS.length} animations, left to right in time</text>`];
