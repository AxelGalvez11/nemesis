...
function DropdownMenuSubContent({
  ...
  return (
    ...
    // `overflow` clip. Radix Popper still anchors it to the SubTrigger.
    <DropdownMenuPrimitive.Portal>
      <DropdownMenuPrimitive.SubContent
        // Fixed `max-h-80` on purpose: Radix only publishes the available-height
        // variable on Content, NOT SubContent — using it here collapses to 0px.
        className={cn(
          "dt-portal-scrollbar z-[140] max-h-80 min-w-36 origin-(--radix-dropdown-menu-content-transform-origin) overflow-y-auto rounded-lg border border-(--ui-stroke-secondary) bg-[color-mix(in_srgb,var(--ui-bg-elevated)_96%,transparent)] p-1 text-[length:var(--conversation-text-font-size)] text-popover-foreground shadow-md backdrop-blur-md data-[side=bottom]:slide-in-from-top-1 data-[side=left]:slide-in-from-right-1 data-[side=right]:slide-in-from-left-1 data-[side=top]:slide-in-from-bottom-1 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95",
          className,
        )}
        collisionPadding={collisionPadding}
