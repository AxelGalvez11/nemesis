...
test("🔴 every tint stays legible as TEXT, in both themes, on the composer's own fill", () => {
  // 🔴🔴 THIS CHANGE RAISED THE BAR ON THESE TOKENS AND THE BAR HAD TO BE RE-CHECKED. Their own
  // note in `desktop-ui.css` says they are "decoration, not text: these tint 16px glyphs beside a
  // label that carries the meaning in words". The chip now paints the LABEL in them, so they are
  // carrying words for the first time, and a tint that was fine under a glyph could have been
  // unreadable under a word. Measured here rather than assumed.
  //
  ...
  // beside it asks that capability's own question. The gate exists to stop a NEW capability
  // arriving with a tint nobody looked at.
