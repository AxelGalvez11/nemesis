...
// J1/J2 (docs/canvas-v1-acceptance.md, owner architectural correction 2026-08-13): the learner
...
// Canvas. This reads the component's source rather than rendering it, matching the convention
...
test("provenance survives -- the source/link control is untouched", async () => {
  const source = await SOURCE;
  assert.match(source, /icon="link"/);
  assert.match(source, /Where this came from/);
  assert.match(source, /icon="question"/);
});
