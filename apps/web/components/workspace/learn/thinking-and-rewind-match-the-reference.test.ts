...
test("🔴🔴🔴 the running step is a line in the conversation, not a caption on the character", () => {
  ...
  assert.match(
    CANVAS,
    // 🔴 REPOINTED 2026-09-04: `ActivityTrailView` draws the line (and the list of steps under
    // it) in the same slot, and falls back to `CanvasThinkingPreview` until the first step lands.
