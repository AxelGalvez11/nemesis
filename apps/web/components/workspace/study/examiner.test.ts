...
test("🔴 an aced paper offers a HARDER one, gated on actually acing it", () => {
  // Calibration: lower the gate to a plain button on every review — or
  // regenerate at standard difficulty — and these redden. The report's rule is
  ...
  assert.match(dialogs, /testOpts: \{ challenge: "hard" \}/, "the regeneration is no longer hard mode");
...
test("🔴 mixed review spans every deck and note, and carries the misses back", () => {
  ...
  assert.match(dialogs, /reasksMissed: missed\.length > 0/, "the re-ask instruction no longer follows the misses");
});
