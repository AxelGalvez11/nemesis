...
export function SessionRightRail({ panel, onPanelChange, onCollapse, sources, outputs }: { panel: SessionRailPanel; onPanelChange: (panel: SessionRailPanel) => void; onCollapse: () => void; sources: SessionRailSource[]; outputs: SessionOutput[] }) {
  ...
  // Notes first, matching the popup the chat card opens (owner 2026-07-28) —
  // the write-up is what a student came for; the transcript backs it up.
  const [recordingView, setRecordingView] = useState<"transcript" | "notes">("notes");
        ...
        {panel === "sources" ? (
          ...
          <div className="grid gap-4">
            ...
            <div className="grid grid-cols-2 rounded-xl bg-(--ui-bg-quaternary) p-0.5">
              {(["notes", "transcript"] as const).map((view) => <button className={cn("rounded-[0.6rem] px-2 py-1.5 text-xs capitalize text-(--ui-text-tertiary)", recordingView === view && "bg-(--ui-control-active-background) text-foreground shadow-sm")} key={view} onClick={() => setRecordingView(view)} type="button">{view}</button>)}
            </div>
