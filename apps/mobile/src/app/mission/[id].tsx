...
// lines and final result appear without a manual refresh. Review actions are the two
...
export default function MissionDetail() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    Promise.all([getMission(id), listMissionEvents(id)])
      .then(([m, evs]) => {
        if (cancelled) return;
        setMission(m);
        setEvents(evs);
  ...
  const act = async (kind: "reviewed" | "changes" | "cancel") => {
    ...
    try {
      if (kind === "reviewed") {
        ...
        await requestChanges(mission.id, note);
        setNote("");
      } else {
      ...
      {loading ? (
        ...
        <>
          <View style={styles.header}>
            ...
            <StatusPill status={mission.status} label={statusLabel(mission, true)} />
          </View>

          <ScrollView ref={scrollRef} style={styles.flex} contentContainerStyle={styles.feed} testID="mission-event-feed">
