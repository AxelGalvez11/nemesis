...
//
// PORTED VERBATIM from apps/web/lib/workspace/study-occlusion.ts, deliberately
// unchanged. Both clients read the same `study_cards.payload` jsonb, so the
// geometry and the validation have to agree exactly: a mask the phone rounds
// differently would sit somewhere else on the web, and a payload one client
// accepts and the other rejects is a card that only exists on one device.
//
