...
//
// PORT of apps/web/lib/workspace/test-answer-balance.ts, deliberately line-for-
// line so both surfaces lay out a paper identically. The phone needs its own copy
// because the phone's chat now WRITES tests (owner 2026-07-24: "the phone chat
// needs to be able to create flashcards, tests, mindmaps"), and a test written on
// the phone must not carry the bias a test written on the web has had removed.
//
// Language models cluster the correct option's position hard when they write
// multiple-choice questions. You cannot fix that with an instruction: the model
// has no memory of the previous question's index while writing the next one, and
// asking for "randomness" gets you its idea of random, which is the same three or
// four patterns. So the model writes the question and decides which option is
// TRUE, and this module decides WHERE that option sits — the repo's standing rule
// that a computed fact is computed in real code, never guessed by an LLM.
//
// DETERMINISTIC ON PURPOSE. Same questions in, same layout out, on either device.
//
// NEVER RUN THIS OVER A TEST SOMEBODY HAS TAKEN. TestAttempt.missed stores
// `picked` as an option INDEX, so moving the options after an attempt exists would
// rewrite history: the student's recorded answers would point at different text
// than the ones they chose. Call it once, at creation, while `attempts` is empty.
