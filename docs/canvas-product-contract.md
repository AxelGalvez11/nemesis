...
- **A harvested shelf of named collections** (`reference-shelf.ts`, generated by
  ...
  biology, microbiology, chemistry, astronomy, physics), the Blausen Medical gallery, the CDC
  image library uploads and NHGRI's genome illustrations — thousands of rows, every single file's
  licence read and normalised exactly as a hand row's is, refusals counted in the file's own
  header. Collections are named in code, so adding one is a reviewed change and never a crawl;
  `reference-shelf.test.ts` re-verifies every row offline on every run. Hand rows are listed ahead
  of shelf rows, and both ahead of the live provider. Two collections were harvested and then
  **removed by the owner the same day** — Servier Medical Art (icon-grade clip art, not teaching
  diagrams) and Gray's Anatomy plates (1918 engravings; Blausen is the anatomy set) — and the
  shelf's tests assert they stay removed; the live provider can still surface such a file when
  nothing newer matches, because a cut from the shelf removes the preference, never the licence.
