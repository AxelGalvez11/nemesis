...
| 2 — PDF native + visual | NOT STARTED | No | **🔴 BASELINE MEASURED 2026-08-06 over 120 real course PDFs / 952 pages: 326 pages (34.2%) are text-rich AND carry a figure, holding 1,807 figures across 80 of the 120 files.** Production's routing rule guarantees none of them is ever looked at, and `pdfCoverage` accepts no figure input, so the loss is invisible to the honesty layer too. Script: `apps/web/scripts/phase2-figure-baseline.mts`. See `docs/document-benchmark.md`. | Depends on Phase 1. The mechanism is the blocker as much as the rule: `unpdf` does not expose image operators at all, so seeing these figures requires the pdf.js-based extraction Phase 2 exists to build. |
| 3 — Office fidelity | NOT STARTED | No | **🔴 BASELINE MEASURED 2026-08-06 over 124 real Word documents / 16,997 paragraphs: 8,355 table cells (198 tables, 57 files) each become an orphan line; 2,266 numbered paragraphs in 76 files (61%) lose their numbering entirely; 123 headings, 116 hyperlinks and 149 figures dropped. 103 of 124 files (83%) lose structure.** Worst file reaches the model as 1,860 unlabelled lines. PPTX by contrast is genuinely strong (notes, SmartArt, charts, tables, TIFF/EMF, glyph filtering) and must not regress. Script: `apps/web/scripts/phase3-docx-baseline.mts`. | Depends on Phase 2's canonical model. Slide rendering needs an owner decision. Numbering is the hardest of these: it is not degraded but absent, and unlike headings it cannot be inferred from the text — `numbering.xml` has to be opened and resolved. |
