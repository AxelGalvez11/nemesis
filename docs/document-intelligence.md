...
| 1 — Document worker | IN PROGRESS | **All of it, as of 2026-08-07.** Schema + RPCs (lease, backoff, recovery cron), status resolver, enqueue decision, shared parser — and now the `worker_thread` entry (`parse-thread.ts`), the run orchestration with lease/deadline/memory guards (`parse-run.ts`), the worker route, the student-facing kick+status route, and the status banner. | *A parse runs off the request thread; the lease is renewed while it runs; the run is abortable; and the worker bundle is shipped with the function.* → **Yes, all four.** The heartbeat test spawns a real thread, blocks it with 1,200 ms of solid CPU and counts renewals that actually fired (inline this number is 0 — that is the whole finding). The real esbuild bundle parses a real PDF on a real thread, so a bundle that silently lost its parser fails the suite. `check-worker-trace.mjs` reads the built function's own trace manifest: `workers/parse-thread.mjs`, 1 of 100 traced files. | **Not merged** · Not deployed · Not production-proven. | **Whether the bundle LOADS in a deployed function.** A trace entry proves the bytes shipped; only a real invocation proves the runtime can spawn them. Queued as handoff §6. Per-unit retry is **deferred with owner approval** — DOCX has no reproducible unit identity before Phase 2 — so §6.4's "a failed page or batch retries without restarting the document" **cannot be met in this phase**, and Phase 1 must not be marked DONE against it. |
| **Canonical model** (prerequisite, added 2026-08-06) | IN PROGRESS | `packages/shared/src/document-model.ts`. Units → blocks, with the no-fabricated-locator rule made **structural**: a unit carries its own kind and `describeLocator` refuses to say "page" about one that is not. Produced by DOCX and PDF, consumed by `parseDocument`, persisted as a `v2 units-blocks` envelope. | *Everything the readers recover survives the model boundary, and no locator is invented.* → **Yes**, on 124 real Word files: 111 headings, 2,116 list items, 197 tables, 8,345 cells — all exact — across 5,227 blocks, **0 fabricated locators**. | Not merged · Not deployed · Not production-proven. | Implied by Phases 2–5 and owned by none of them, which is why Phase 3's structure had nowhere to go. Built out of numbered order and recorded here rather than done silently. |
| 2 — PDF native + visual | IN PROGRESS | `apps/web/lib/pdf/structure.ts` + pure `geometry.ts`, wired into `parseDocument` and the upload route. Transform-stack replay for figure boxes; reading order; column detection; heading levels from relative type size. | *A figure on a text-rich page is found and located.* → **Yes**: over the same 120 real course PDFs / 952 pages as the baseline, **1,963 figures found where production sees none** — 239 running art, 635 furniture, leaving **1,089 nobody has examined**. 0 rects outside 0..1, **0 words lost against `unpdf`**. *A found figure is actually looked at.* → **NO. Not built.** | Not merged · Not deployed · Not production-proven. | **The visual half.** Production routes vision by text sparsity alone, so a text-rich page with a diagram is never inspected — 326 pages (34.2%) holding **1,807 figures** across 80 files, which is the standing baseline. Also unbuilt: table detection from geometry (deliberately not approximated — a grid asserted over prose relabels every value in it), query-time high-resolution reinspection, and the render cache. |
| 3 — Office fidelity | IN PROGRESS | `docx-structure.ts` → `docx-model.ts` → `parseDocument`; header rows read from `<w:tblHeader/>`; OMML equations read from `<m:t>`. | *Word structure reaches storage instead of dying at a function boundary.* → **Yes**, on 124 real files (numbers above). Until 2026-08-06 the reader's output was rendered back to markdown and discarded, so the recovery numbers were true about the reader and false about what any model received. | Not merged · Not deployed · Not production-proven. | **PPTX still produces text, not the model**, and its quality must not regress — baseline 62,040 chars / 56 headings / 807 bullets, which nothing currently re-asserts. Slide rendering needs an owner decision (LibreOffice in a container). |
| 4 — Source retrieval | IN PROGRESS | `packages/shared/src/document-chunks.ts`: a table is never split, a heading opens a chunk and never closes one, and a block over the target stays whole and declares itself `oversized`. Cap audit complete; `MATERIAL_CHAR_LIMIT` now has one definition. | *Chunks preserve the document.* → **Yes**, on 124 real files: 588 chunks, **0 tables split, 0 documents lost text**, asserted directly by a test. *Chunks enter and leave the index.* → **NO. Nothing writes them.** | Not merged · Not deployed · Not production-proven. | `library_chunks where origin_type='source'` = **0**. Only notes are indexed, so "importing a lecture makes it available" is still true only because the librarian turned it into notes. Hybrid/structure-aware ranking does not exist. Every cap in §6.4 is *verified* but not yet *removed*. |
| 5 — Verified citations | IN PROGRESS | `packages/shared/src/document-citations.ts`. The primitive is inverted: a claimed location is never trusted, the quoted text is found and the locator derived. Quotes under 24 characters refused; ambiguous quotes refused rather than pointed at the first match; `resolveLocator` checks unit kind against the document. 11 tests. | *A fabricated locator cannot survive verification.* → **Yes**, by unit test. *A citation reopens the correct evidence.* → **NO.** Nothing calls this, and there is no ground truth to score against. | Not merged · Not deployed · Not production-proven. | No consumer in chat or the reader, so model-supplied locations are still stored unchecked. No region crop for a PDF rect. **No labelled ground-truth set exists** — that is a deliverable, not a side effect of the code. |
| 6 — Semantic extractors | IN PROGRESS | **The extractors are implemented and tested; the phase is not.** `packages/shared/src/document-facts.ts`: outline, definitions, procedures, dates, weightings, and an inventory of figures/tables/equations — every one keyed on **shape, never subject vocabulary**. A grading scheme is found without the word *grading*: a table column of shares summing to a whole. 34 tests. | *Facts are found by shape and carry provenance.* → **Yes**: 472 facts over 94 of 124 real documents, all with valid provenance, none claiming a page. Two fixtures, one law and one welding, share no vocabulary and yield the same fact kinds. Real files cut definitions **925 → 265 → 89**. *Academic artifacts are derived from those facts.* → **NO. Nothing consumes them.** | Not merged · Not deployed · Not production-proven. | 🔴 **The phase's actual point is unbuilt**: no note, deck, test, or calendar entry is built from a fact, so nothing yet inherits provenance from a source block. Also carried: `EXAM_ITEM_RULES` writes clinical vignettes, which fails the law-student / mechanical-engineering test and is a correctness bug belonging to this phase. Known gaps recorded rather than fixed: English month names only; a trailing letter is an enumerator in *Answer D* and a real term in *Vitamin D*. |
| 7 — Capacity | IN PROGRESS | `MAX_UNITS_PER_PARSE` now **enforced** in `readPdfStructure`, which reports `declaredUnits` separately. `ArchiveRefusal` so our own refusals survive wrapping. 18 hardening tests. | *Adversarial input is bounded and disclosed, not crashed on.* → **Yes, for the parsers in isolation**: three defects found and fixed — a PDF that would not open threw out of the parser and took the upload request with it; fflate's `invalid zip data` reached the student verbatim; coverage printed `5000 of 100000` beside a sentence that grouped its digits. *The integrated pipeline is bounded.* → **NO.** The fixtures were run through `readPdfStructure`, not through parse → chunk → index → retrieve. | Not merged · Not deployed · Not production-proven. | Re-run against the integrated pipeline once Phase 4 has one. Also unbuilt: bounded entry-at-a-time Office reading, streaming inflation to close the understated-`originalSize` gap, and concurrency/memory behaviour under the real runtime. |
