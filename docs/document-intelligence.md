...
| Immediate — chat handoff truncation | BLOCKED | **No — merged, not deployed.** `0a30b805` (#443). | Reproduced on the owner's real 9.6 MB / 57-slide deck: cut at slide 55, model reported slide 46. Fix verified locally: 1293 web tests pass, `tsc` clean; the regression test carries its own proof that the old ceiling drops slide 57. | Vercel daily build cap. The code is on `main` and cannot reach production until it clears. **Merged is not live.** |
| 1 — Document worker | IN PROGRESS | No | Runtime chosen by measurement (`docs/document-worker-spike.md`): Supabase Edge disqualified at 959 MB Deno / 808 MB Node against a 256 MB cap. Lease heartbeat 0/49 inline vs 50/50 in a `worker_thread`. **Claim predicate measured against production before applying: the original matched 16 of 17 existing sources; with the enqueue gate, 0.** Built and tested: schema + RPCs, status resolver, enqueue decision, shared parser. 1326 tests, `tsc` clean. | **The `worker_thread` entry point.** The spike's decision is measured and stands — a lease cannot be renewed from the parsing thread, and its own note rejects the tempting shortcut ("the TTL must exceed the worst document any student ever uploads — a number we do not control"). But a worker thread does not inherit the TypeScript loader, so the entry has to be a separately-traced file, and **whether it is bundled into the deployed function cannot be checked while the build cap blocks deploys.** Writing one that passes locally and may not load in production would be exactly the designed-vs-proven confusion §6.7 exists to prevent. Also unbuilt: the kick route and the status UI. Per-unit retry is **deferred with owner approval** — DOCX has no reproducible unit identity before Phase 2 — so §6.4's "a failed page or batch retries without restarting the document" **cannot be met in this phase**, and Phase 1 must not be marked DONE against it. |
| **Canonical model** (prerequisite, added 2026-08-06) | IN PROGRESS | No | `packages/shared/src/document-model.ts`. Units → blocks, with the no-fabricated-locator rule made **structural**: a unit carries its own kind and `describeLocator` refuses to say "page" about one that is not, so no code path can print a page for a Word document. Produced by DOCX and PDF, consumed by `parseDocument`, persisted as a `v2 units-blocks` envelope in `parsed_documents.structure`. Measured on 124 real Word files: everything the reader recovers survives the model boundary (111 headings, 2,116 list items, 197 tables, 8,345 cells — all exact), 5,227 blocks, **0 fabricated locators**. | This was implied by Phases 2, 3, 4 and 5 and owned by none of them, which is why Phase 3's structure had nowhere to go. Built first, out of the numbered order — recorded here rather than done silently. Not deployed; SOLO not run. |
| 2 — PDF native + visual | IN PROGRESS | No | **🔴 BASELINE MEASURED 2026-08-06 over 120 real course PDFs / 952 pages: 326 pages (34.2%) are text-rich AND carry a figure, holding 1,807 figures across 80 of the 120 files.** Production's routing rule guarantees none of them is ever looked at, and `pdfCoverage` accepts no figure input, so the loss is invisible to the honesty layer too. Script: `apps/web/scripts/phase2-figure-baseline.mts`. **FIX BUILT AND MEASURED 2026-08-06** (`apps/web/lib/pdf/structure.ts` + `geometry.ts`, wired into `parseDocument` and the upload route): over the same 120 files / 952 pages, **1,963 figures found where production sees none** — 239 running art, 635 furniture, leaving **1,089 real figures nobody has examined**, which is the first time this system has been able to state that number at all. Zero fabricated locators, zero rects outside 0..1, and **no document lost words against `unpdf`**. Coverage gained a `not-examined` figure reason that counts as lost, so a page whose text was read but whose diagram was not is now `partial`. Script: `apps/web/scripts/phase2-structure-check.mts`. | Not deployed; SOLO not run. **Still unbuilt within the phase:** table detection from geometry (deliberately not approximated — a grid asserted over prose relabels every value in it), adaptive vision spend on the 1,089, query-time high-resolution reinspection, and the render cache. Two column rules were wrong until real geometry corrected them, which is recorded in `docs/document-benchmark.md`. |
| 3 — Office fidelity | IN PROGRESS | No | **🔴 BASELINE MEASURED 2026-08-06 over 124 real Word documents / 16,997 paragraphs: 8,355 table cells (198 tables, 57 files) each become an orphan line; 2,266 numbered paragraphs in 76 files (61%) lose their numbering entirely; 123 headings, 116 hyperlinks and 149 figures dropped. 103 of 124 files (83%) lose structure.** Worst file reaches the model as 1,860 unlabelled lines. PPTX by contrast is genuinely strong (notes, SmartArt, charts, tables, TIFF/EMF, glyph filtering) and must not regress. Script: `apps/web/scripts/phase3-docx-baseline.mts`. **The structure now survives**: until 2026-08-06 the reader's output was rendered back to markdown and discarded at the function boundary, so the recovery numbers were true about the reader and false about what any model received. `extractDocxModel` produces the canonical model and `parseDocument` returns it. Real files found one more thing fixtures could not: **only 3 of 197 tables actually mark a header row**, and the old renderer drew a header separator on all 197 — promoting a data row to a column name in 98% of real tables. | Not deployed; SOLO not run. **Still unbuilt within the phase:** PPTX still produces text rather than the model (its quality must not regress — baseline 62,040 chars / 56 headings / 807 bullets), and slide rendering needs an owner decision. |
| 4 — Source retrieval | IN PROGRESS | No | Baseline: `parsed_documents` = 1 row; `library_chunks where origin_type='source'` = 0. Only notes are indexed. **Cap audit run 2026-08-06: every cap in the table above verified against the code at the stated line.** `MATERIAL_CHAR_LIMIT` was genuinely defined twice and is now imported from one place — the two copies had been held together by a test that string-matched the other file's source for the literal, which is a drift alarm rather than a single source of truth. **Structure-aware chunking built 2026-08-06** (`packages/shared/src/document-chunks.ts`): a table is never split, a heading opens a chunk and never closes one, and a block over the target stays whole and declares itself `oversized` rather than being split or dropped. Verified on the 124 real Word documents — 588 chunks, **0 tables split across chunks, 0 documents lost text** — and a unit test asserts loss-freeness directly, since never deleting content silently is only a rule if something checks it. | Not deployed; SOLO not run. **Still unbuilt:** nothing writes these chunks to `library_chunks`, so source retrieval is still notes-only, and hybrid/structure-aware ranking does not exist. The remaining caps are *verified* but not yet *removed*: each still silently bounds how much of a source is understood, which is the phase's actual work. |
| 5 — Verified citations | IN PROGRESS | No | `packages/shared/src/document-citations.ts`. The primitive is inverted: a claimed location is never trusted, the quoted TEXT is found and the locator derived from the document. A quote under 24 characters is refused (it matches everywhere, which looks exactly like matching correctly), a quote appearing twice is refused rather than pointed at the first, and `partitionCitations` returns refusals alongside survivors so a partly-checked answer cannot be presented as a fully checked one. A locator cannot verify itself: `resolveLocator` checks the unit kind against the document, so a stored locator claiming a page for a `.docx` fails. 11 tests. | Not deployed; SOLO not run. **Still unbuilt:** nothing in chat or the reader calls this yet, so model-supplied locations are still stored elsewhere; and there is no region crop for a PDF rect. |
| 6 — Semantic extractors | IN PROGRESS | No | `packages/shared/src/document-facts.ts`. Outline, definitions, procedures, dates, weightings, and an inventory of figures/tables/equations — **every one keyed on SHAPE, never on subject vocabulary**, each carrying a locator built the same way a citation is. A grading scheme is found without the word *grading*: a table column of shares summing to a whole. Two fixtures, one law and one welding, share no vocabulary and produce the same fact kinds. **Real files cut the definition extractor twice: 925 → 265 → 89.** The colon separator ran ~4 false positives per true one (form fields, worksheet prompts, case labels) and was removed on that evidence; a single dashed line is prose, so a run of three consecutive siblings is now required. 472 facts over 94 of 124 real documents, **all with valid provenance, none claiming a page**. 34 tests. | Not deployed; SOLO not run. **Still unbuilt:** nothing consumes these facts — no study material, calendar entry or answer is built from them yet. Known gaps recorded rather than fixed: English month names only (numeric/ISO forms are language-independent), and a trailing letter is an enumerator in *Answer D* and a real term in *Vitamin D*. |
| 7 — Capacity | IN PROGRESS | No | Measured on the real 123.8 MB deck: old fetch path peaked at 676 MiB RSS (5.73×); after #434/#435, 391 MiB (3.31×). Parse side still holds the whole archive (+121 MiB, 99.7% of it media). **`MAX_UNITS_PER_PARSE` is now ENFORCED** — it was declared, asserted about, and applied nowhere. `readPdfStructure` bounds the read and reports `declaredUnits` separately, so coverage says "5,000 of 100,000 pages could be read" instead of renaming the document to its own prefix. 18 hardening tests over adversarial inputs found three further defects, all fixed: a PDF that would not open **threw out of the parser and took the upload request with it**; fflate's `invalid zip data` reached the student verbatim for any truncated or mislabelled archive; and coverage printed `5000 of 100000` while the truncation sentence beside it grouped its digits. | Not deployed; SOLO not run. **Still unbuilt:** bounded entry-at-a-time Office reading (`docs/source-ingestion-jobs.md`), streaming inflation to close the understated-`originalSize` gap, and concurrency/memory behaviour under the real runtime. |
