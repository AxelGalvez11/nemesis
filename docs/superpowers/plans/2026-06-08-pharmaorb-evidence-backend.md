...
- **P3 — Agentic answer engine.** Router + tool loop (vector_search/pubmed_search/clinicaltrials_search/get_entity_evidence/rerank); corrective/self-RAG; conversation-aware retrieval; streaming. Exit: MVP cited-answer quality, eval-proven; safety guardrail suite still green.
  - **Delivery-path eval (do NOT skip):** how retrieved chunks reach the model — inline-in-context vs written-to-file/tool-result the agent must re-read — moves answer accuracy as much as the retrieval method itself, and can *invert* which retriever wins. Source: *"Is Grep All You Need?"* (arXiv 2605.15184, 2026): same model + same data, ~16-pt accuracy swings by harness; Codex grep collapsed 93.1%→55.2% purely from switching inline→file-based delivery. **Implication:** P3's eval must vary `retrieval × harness/tool-calling style × delivery-path` jointly (extend the answer-eval harness to A/B delivery), not retrieval in isolation — retrieval, orchestration, and delivery are one system. **Corollary (same paper):** the authors explicitly flag "scientific synthesis over paraphrased abstracts" as a domain where dense/hybrid beats grep — that's us, so keep dense as the core; lexical/FTS (PR2) handles only the verbatim slice (NCT/PMID/exact dose/drug codes like BPC-157).
