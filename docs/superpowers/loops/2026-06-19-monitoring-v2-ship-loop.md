...
**Current slice:** A — Drug picker · **Status:** picker built + wired + reviewer-hardened + **runtime-verified in preview** (real component, all 3 interaction paths + the re-fetch fix proven via live ARIA state); advisor (slice-done checkpoint) → STAGE prod deploy (hard stop) · **Last green commit:** `EntityPicker` in the Monitor box
...
- PENDING: advisor (slice-done) → STAGE prod deploy (owner greenlight = the hard stop). Reframed ask: the picker improves how watches are CREATED, but created watches stay inert until the Vault `watch_service_role_key` is synced (scheduler is 401-dead) — recommend owner sync the key FIRST (also fixes the existing empty-watch complaint).

### Slice A2 — Universal picker
- [ ] **A2-0. GAP from A (fold in here): brand-scoping is incomplete on pick.** `watchFieldsFromEntity` sets `mentions = [name, subtitle]`, but `search_entities` returns the subtitle as a SINGLE brand alias (RPC `LIMIT 1`). So picking "semaglutide" scopes openFDA to one brand (e.g. Ozempic) and misses Wegovy/Rybelsus — a catalyst on a sibling brand wouldn't match. Fix: on pick, `fetchDrug(id)` → fold the full `brand_names[]` into `mentions` (dedup, drop the comma-joined subtitle string). Verify the openFDA name-scope then covers all brands. (Cosmetic-only today: created watches are inert until B0's Vault key is synced, so no missed alert has shipped — but fix before Catalysts go live.)
