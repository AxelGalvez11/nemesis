...
**Current slice:** A — Drug picker · **Status:** ✅ DONE + DEPLOYED — picker (`pharma-bro-80k95i1ps`) AND the owner-requested current-evidence view (`pharma-bro-amczljatq`, @ `253d9d7`) are LIVE on app.pharmaorb.app, both verified in the shipped bundle. Scheduler 401→200 fixed (B0). **NEXT:** Slice A2 (universal MeSH picker — A2-1 research done, A2-2 route next) or Slice B (catalysts — B0 prereq satisfied; B1 migration is owner-gated). · **Last green commit:** `253d9d7` current-evidence + Ask `?q=` pre-fill
