...
> 🔴 **THIS RECORD REACHED `main` ON 2026-08-13, TWO DAYS LATE, VIA A DIFFERENT PR.**
> It was written for **#483, which is still open.** The *run* it describes landed in
> **#485 (merged)**, so for two days `main` carried the results of a benchmark whose
> baseline record was invisible — and `parser-benchmarks.md` beside it said
> **"NOT RUN"**. A team lead read that and repeated it to the owner.
>
> **A measurement is not delivered until its record is on `main`.** Superseded on the
> repaired dimensions by `parser-repairs-before-after.md`; kept as the baseline the
> repairs are measured against.
