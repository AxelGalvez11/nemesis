...
Deno.test("a value at the reference median lands at the 50th percentile", () => {
  // male VO2 p50 = 45 → exactly median.
  assertEquals(percentileFor(METRICS.vo2max, 45, "male", "30-39"), 50);
});

Deno.test("higher-is-better metric: a higher value ranks higher", () => {
  const low = percentileFor(METRICS.vo2max, 39, "male", "30-39"); // p25
  const high = percentileFor(METRICS.vo2max, 51, "male", "30-39"); // p75
  assertEquals(low, 25);
  assertEquals(high, 75);
  assert(high > low);
});

Deno.test("lower-is-better metric is inverted: a low HbA1c ranks HIGH", () => {
  // HbA1c p25 raw = 5.0 → raw percentile 25 → oriented 75 (good).
  assertEquals(percentileFor(METRICS.hba1c, 5.0, "male", "30-39"), 75);
  // HbA1c p75 raw = 5.5 → raw 75 → oriented 25 (worse).
  assertEquals(percentileFor(METRICS.hba1c, 5.5, "male", "30-39"), 25);
});

Deno.test("piecewise interpolation between anchors is monotonic", () => {
  const a = percentileFor(METRICS.apob, 74, "male", "30-39"); // p25 (lower better → 75)
  const mid = percentileFor(METRICS.apob, 81, "male", "30-39"); // between p25 and p50
  const b = percentileFor(METRICS.apob, 88, "male", "30-39"); // p50 → 50
  assertEquals(a, 75);
  assertEquals(b, 50);
  // lower ApoB is better, so as raw value rises the oriented percentile falls.
  assert(a > mid && mid > b);
});
...
Deno.test("sex-specific references differ (female grip uses the female curve)", () => {
  // 31 kg is the female p50 but well below the male p10 → very different percentiles.
