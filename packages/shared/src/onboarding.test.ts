// Rules for first-run setup, shared by web and phone. Ported verbatim from
// apps/web/lib/workspace/onboarding.test.ts when the rules moved to this package
// — same assertions, so the move is provably behaviour-preserving.
//
// node:test rather than Deno.test: this folder runs BOTH flavours (Deno supports
// node:test), and keeping the assertions byte-identical to the web original is
// worth more here than matching the neighbouring files' style.
...
/** The only field assignCourse touches is `course`; everything else is padding
 *  so the object looks like a real event. Typed locally rather than imported,
 *  which is exactly what letting assignCourse be generic bought us. */
interface TestEvent {
  date: string;
  id: string;
  kind: string;
...
function event(id: string, course?: string): TestEvent {
