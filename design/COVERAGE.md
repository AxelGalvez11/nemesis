...
| `components/space` | the pages workspace: sidebar, pages, blocks, databases, share, settings; the client itself lives in apps/web/space | measured on Notion's web app; ICONS.md: one library (Lucide), stroke 1.5; INTERACTIONS.md Reversible state, guarded by `apps/web/lib/space/space-sidebar.test.ts`; docs/space/PLAN.md | measured reference |
