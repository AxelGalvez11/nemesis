...
Deno.test("two drugs (interaction) → both OR'd across generic+brand", () => {
  const q = openFdaSearch("lisinopril spironolactone", [
    "lisinopril",
    "spironolactone",
  ]);
  assertEquals(
...
Deno.test("never falls back to bare full-text when a drug was named", () => {
  ...
  assert(
    q !== null,
    "a named drug must produce a field-scoped query, never null",
  );
  assert(
    q.includes("openfda.generic_name:") && q.includes("openfda.brand_name:"),
  );
...
Deno.test("multi-word drug name is phrase-quoted (no loose-token match)", () => {
  ...
  assert(q !== null);
  assert(
    q.includes(`"insulin glargine"`),
    "multi-word name must stay a quoted phrase",
  );
});
...
Deno.test("a name containing a quote is escaped (no query injection)", () => {
  ...
  assert(
    q.includes(`"a\\"b"`),
    "embedded quote must be escaped via JSON.stringify",
  );
});
...
Deno.test("isSafetyCriticalQuery routes toxic/lethal/recall questions to safety lanes", () => {
  ...
  assertEquals(
    isSafetyCriticalQuery("what are common metformin side effects?"),
    false,
  );
});
...
Deno.test("ClinicalTrials is skipped for consumer-product energy-drink safety queries", () => {
  ...
  assertEquals(
    shouldFetchClinicalTrials(
      "caffeine",
      ["caffeine"],
      "caffeine alertness clinical trial",
    ),
    true,
  );
});

Deno.test("OpenAlex is skipped for everyday scalp-flaking clarification queries", () => {
  assertEquals(
    shouldFetchOpenAlex(
      "dandruff / scalp flaking",
      [],
      "white flakes hair dandruff seborrheic dermatitis scalp flaking",
    ),
    false,
  );
  assertEquals(
