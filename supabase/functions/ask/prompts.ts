...
/** Full system prompt for a generation = base + (optional) register style + conversational voice + formatting + (optional gated) claim-check + register-safety floor + intent guidance. */
export function generateSystem(intent: Intent, style?: AnswerStyle, opts?: { verifyClaim?: boolean }): string {
  ...
  // Claim-check block is gated (default off ⇒ ""), and placed BEFORE the safety floor so REGISTER_SAFETY
  // still comes last. Byte-identical composition when the gate is off.
  const claimBlock = opts?.verifyClaim ? CLAIM_CHECK : "";
  const extra = INTENT_GUIDANCE[intent];
  return [BASE_GENERATE_SYSTEM, styleBlock, voiceBlock, formatBlock, claimBlock, safetyBlock, extra].filter(Boolean).join("\n\n");
}
