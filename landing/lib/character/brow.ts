...
// DOM and so the phone renderer can draw the same brow from the same numbers rather than
// guessing at them — the same reason `pool.ts` and `stations.ts` sit here.

import { EYE_H, EYE_W } from "../bloub/face";
import { TAU, clamp } from "../bloub/math";
